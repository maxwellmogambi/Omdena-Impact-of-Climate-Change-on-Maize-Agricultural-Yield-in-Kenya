...
input_defaults = {
    ...
    'total_precipitation_sum (mm)': 0.0,
    'u_component_of_wind_10m (m/s)': 0.0,
...
# Column 1 inputs
with col1:
    st.session_state['temperature_2m (°C)'] = st.number_input("Temperature 2m (°C)", value=st.session_state['temperature_2m (°C)'])
    st.session_state['temperature_2m_max (°C)'] = st.number_input("Max Temperature 2m (°C)", value=st.session_state['temperature_2m_max (°C)'])
    st.session_state['temperature_2m_min (°C)'] = st.number_input("Min Temperature 2m (°C)", value=st.session_state['temperature_2m_min (°C)'])

# Column 2 inputs
with col2:
    st.session_state['total_precipitation_sum (mm)'] = st.number_input("Total Precipitation (mm)", value=st.session_state['total_precipitation_sum (mm)'])
    st.session_state['u_component_of_wind_10m (m/s)'] = st.number_input("U Component of Wind 10m (m/s)", value=st.session_state['u_component_of_wind_10m (m/s)'])
    st.session_state['precipitation (mm)'] = st.number_input("Precipitation (mm)", value=st.session_state['precipitation (mm)'])
